...
const uri = `mongodb+srv://${process.env.DB_USER}:${process.env.DB_PASS}@cluster0.eyq1y.mongodb.net/?retryWrites=true&w=majority`;
...
async function run() {
    try {
        ...
        const productCollection = client.db('bike_parts').collection('products');
        ...
        app.put('/user/:email', async (req, res) => {
            ...
            const user = req.body;
            const filter = { email: email };
            const emailSenderOptions = { upsert: true };

            const updateDoc = {
                $set: user,
            };
            const result = await userCollection.updateOne(filter, updateDoc, emailSenderOptions);
...
app.get('/', (req, res) => {
    res.send('Hello From Bike Parts Server!')
})
